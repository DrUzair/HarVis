DROP PROCEDURE IF EXISTS utubedb_riyadh.Discover_UoD_CoWords2;
...
DELIMITER ##
CREATE DEFINER=`uTube`@`localhost` PROCEDURE `utubedb_riyadh`.`Discover_UoD_CoWords2`(IN authCount INT)
BEGIN	
	...
	SELECT Word, WordCount, AuthorsCount FROM UoD 
				...
				OPEN cowords_cursor;					
					cowords_loop: LOOP 
						...
						IF (intCoOccurenceCount > 0) THEN
							INSERT INTO UoD_Titles (Word, WordCount, AuthorsCount, CoWord, CoWordCount, CoWordAuthorsCount, CoOccurenceCount)
