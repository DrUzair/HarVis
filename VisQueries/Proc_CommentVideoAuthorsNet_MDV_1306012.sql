...
CREATE DEFINER=`uTube`@`localhost` PROCEDURE `CommentVideoAuthorsNet_MDVA`(IN TOP_N_VIDEO_AUTHORS int, IN MIN_COMMENT_AUTHORS_COUNT INT, IN MIN_COMMENTED_VIDEOS_COUNT INT)
BEGIN	
	...
	OPEN video_authors_cursor;		
		...
		videos_authors_loop: LOOP			
			-- FETCH NEXT most Discussed video_author FROM videos_authors_cursor
